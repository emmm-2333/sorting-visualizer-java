...
② 排序动画服务（SortingService）：回调驱动 + 暂停/继续/退出排序
...
6）支持“退出排序”：用户点击“退出排序”后，Controller 会调用 `SortingService.requestCancel()` 并对 Task 执行 `cancel()`；排序线程会在 `sleep()/waitIfPaused()` 处检测取消并快速退出；
7）排序结束后再一次 `updateArray(arrayToSort)` 展示最终结果（若未被取消）。

图2：SortingService 内部监听器处理流程图（含退出排序）

```mermaid
flowchart TD
    A[createSortTask] --> B[Task.call]
  B --> C[生成运行令牌+resume 清除暂停]
    ...
    G --> J[sleep: 检查取消 + waitIfPaused + Thread.sleep(delay)]
    H --> J
    I --> J
    ...
    M[用户点击退出排序] --> N[requestCancel + Task.cancel]
    N --> O[排序线程在sleep/wait检测取消并退出]
```
...
解决办法：暂停不直接中断线程，而是在每一步 sleep 前检查 `paused`，若为 true 则在 `pauseLock` 上等待；为了兼容“退出排序”，等待通常采用带超时的 `wait(50)` 循环，以便周期性检查取消请求；继续时将 `paused=false` 并 `notifyAll()` 唤醒等待线程。
...
④ 可通过“速度滑块”调整动画速度（滑块越大，延迟越小，动画越快；当前为非线性映射，范围约 1ms~1000ms）。
...
① 排序开始后，“退出排序”按钮可用；
② 点击“退出排序”，系统会立即终止当前排序任务，并把显示数据恢复到排序前的状态。
