...
```mermaid
flowchart TD
    A["主方法: launch()"] --> B["start(Stage)方法"]
    B --> C["FXMLLoader加载MainLayout.fxml"]
    C --> D["创建场景Scene(1200x800)"]
    D --> E["设置窗口标题"]
    E --> F["设置场景到舞台"]
    F --> G["显示窗口"]
```
...
功能：响应按钮事件（生成数据/开始排序/暂停继续/退出排序/性能比较/显示自定义数据/读取文件），并根据运行状态启用/禁用控件。

关键状态字段：

- `int[] currentArray`：当前显示/排序的数据
- `Task<Void> currentSortTask`：当前排序任务
- `long delay`：动画延迟（由速度滑块计算得出；当前为非线性映射，范围 1ms~1000ms）

（补充：为支持“退出排序并恢复”，控制器还会保存 `arrayBeforeSort` 快照，并用 `exitRequestedByUser` 标记取消来源。）
...
- 对“正在运行”的排序任务发起协作式取消：先调用 `sortingService.requestCancel()`（解除暂停并触发取消标志），再 `currentSortTask.cancel()`。
- 任务 `onCancelled` 时恢复按钮状态，并把 `currentArray` 恢复为排序前快照。

5）`onBenchmark()`：创建性能比较任务并展示结果
...
flowchart TD
  ...
  B -->|生成数据| C[onGenerateData方法]
  C --> C1[读取数据规模/类型]
  C1 --> C2[DataGenerator生成数组]
  C2 --> C3[VisualizerPane.setArray方法]
  C3 --> C4[状态栏提示]

  B -->|显示数据| D[onShowData方法]
  D --> D1[DataInputService.parseInputString方法]
  D1 --> D2[VisualizerPane.setArray方法]

  B -->|读取文件| E[onLoadFile方法]
  E --> E1[FileChooser选择文件]
  E1 --> E2[DataInputService.loadFromFile方法]
  E2 --> E3[VisualizerPane.setArray方法]

  B -->|开始排序| F[onSort方法]
  F --> F1[SortingService.createSortTask方法]
  F1 --> F2[后台线程执行任务]
  F2 --> F3[完成/失败恢复控件]

  B -->|暂停/继续| G[onPauseResume方法]
  G --> G1[SortingService.pause/resume方法]

  B -->|退出排序| X[onExitSort方法]
  X --> X1[SortingService.requestCancel + Task.cancel]
  X1 --> X2[Task.onCancelled恢复UI并回退数组]

  B -->|性能比较| H[onBenchmark方法]
  H --> H1[BenchmarkService.createBenchmarkTask方法]
  H1 --> H2[后台线程执行]
  H2 --> H3[TabPane展示图表/表格]
```
...
- 退出排序采用“协作式取消”：在 `sleep()` / `waitIfPaused()` 处检查取消标志并快速退出；并通过运行令牌屏蔽旧任务残留的 UI 更新

图3：SortingService 内部监听器处理流程图（含退出排序）

```mermaid
flowchart TD
    A["createSortTask方法"] --> B["Task.call()方法"]
  B --> C["生成运行令牌+resume清除暂停"]
    C --> D["AlgorithmRegistry.getSorter()方法"]
    D --> E["arrayToSort = data.clone()方法"]
    E --> F["sorter.sort(arrayToSort, listener)方法"]

    F -->|"onCompare()回调"| G["Platform.runLater: 高亮红色"]
    F -->|"onSwap()回调"| H["Platform.runLater: 更新数组并高亮绿色"]
    F -->|"onSet()回调"| I["Platform.runLater: 更新数组并高亮蓝色"]

    G --> J["sleep: 检查取消/等待暂停/线程休眠(delay)"]
    H --> J
    I --> J

    J --> F

    M["用户点击退出排序"] --> N["SortingService.requestCancel + Task.cancel"]
    N --> O["排序线程在sleep/wait处检测到取消并退出"]

    F --> K["排序结束"]
    K --> L["Platform.runLater: 更新数组(最终)"]
```
...
flowchart TD
  ...
  C["算法回调sleep()方法"] --> D[waitIfPaused方法]
  D --> E{paused为真?}
  E -->|是| F["pauseLock.wait(50)循环等待(可检查取消)"]
  E -->|否| G["Thread.sleep(delay)方法"]

  H[用户点击继续] --> I[resume方法]
  I --> J[paused=false]
  J --> K[pauseLock.notifyAll方法]
  K --> L[等待线程苏醒继续执行]

  M[用户点击退出排序] --> N["requestCancel(): cancelRequested=true"]
  N --> O["resume(): notifyAll唤醒等待线程"]
  O --> P[排序线程检测到取消并退出]
```
...
```mermaid
flowchart TD
  A[选择文件] --> B{"是否选择文件(file为null)?"}
  B -->|是| C[抛 IllegalArgumentException: 未选择文件]
  B -->|否| D[使用 BufferedReader 逐行读取]
  D --> E[使用 StringBuilder 拼接内容]
  E --> F["调用 parseInputString(sb)"]
  F --> G["返回解析后的 int[]"]
```
...
```mermaid
flowchart TD
  A[输入参数：数据大小、数据类型、算法列表] --> B[生成基准数据 baseArray]
  B --> C[遍历算法列表]
  C --> D[获取算法实例和元数据]
  D --> E[复制基准数据到 arrayCopy]
  E --> F["执行 System.gc() 和 sleep(100)"]
  F --> G[记录开始内存和时间]
  G --> H["执行算法排序 (listener = null)"]
  H --> I[记录结束内存和时间]
  I --> J[计算耗时和内存使用量]
  J --> K["添加到结果列表"]
  K --> C
...
```mermaid
flowchart TD
  A[类加载 AlgorithmRegistry] --> B[静态初始化块]
  B --> C["注册冒泡排序"]
  C --> D["注册快速排序"]
  D --> E[...依次注册其他算法]
  E --> F[sorters: 算法名->算法实例]
  E --> G[metadata: 算法名->算法信息]
...
```mermaid
flowchart TD
  A["分区函数 partition(low, high)"] --> B["选择 pivot = array[high]"]
  B --> C[i = low-1]
  C --> D[for j=low..high-1]
  D --> E["触发比较回调 onCompare(j, high)"]
  E --> F{"array[j] < pivot ?"}
  F -->|否| D
  F -->|是| G[i++]
  G --> H["交换 array[i], array[j]"]
  H --> I["触发交换回调 onSwap(i, j)"]
  I --> D
  D --> J["交换 array[i+1], array[high]"]
  J --> K["触发交换回调 onSwap(i+1, high)"]
  K --> L[返回分区点 i+1]
```
...
```mermaid
flowchart TD
  A["合并函数 merge(l,m,r)"] --> B[构造临时数组 L/R]
  B --> C[i=0,j=0,k=l]
  C --> D{ i<n1 && j<n2 ? }
  D -->|否| H[复制剩余元素]
  D -->|是| E["触发比较回调 onCompare(l+i, m+1+j)"]
  E --> F{"L[i] <= R[j]?"}
  F -->|是| G1["array[k]=L[i]; 触发设置回调 onSet(k,L[i]); i++"]
  F -->|否| G2["array[k]=R[j]; 触发设置回调 onSet(k,R[j]); j++"]
  G1 --> G3[k++]
  G2 --> G3
  G3 --> D
  H --> I[while i<n1: 写回并触发 onSet]
  I --> J[while j<n2: 写回并触发 onSet]
  J --> K[返回]
```
...
```mermaid
flowchart TD
  A["堆化函数 heapify(n, i)"] --> B[largest=i]
  B --> C[l=2i+1, r=2i+2]
  C --> D{l < n ?}
  D -->|是| E["触发比较回调 onCompare(l, largest)"]
  ...
  H --> J
  I --> J
  J -->|是| K["触发比较回调 onCompare(r, largest)"]
  ...
  O --> P
  P -->|否| Q[返回]
  P -->|是| R["交换 array[i], array[largest]"]
  R --> S["触发交换回调 onSwap(i, largest)"]
...
- 扫描 max/min 时：用 `onCompare(i,i)` 提供视觉反馈
...
```mermaid
flowchart TD
  A[BucketSort.sort] --> B[扫描数组找最大最小值]
  B --> C["桶数量 = max(1, sqrt(n))"]
  C --> D[初始化桶列表]
  D --> E["计算范围 = (max-min+1)/桶数量"]
  E --> F[遍历数组分配元素到桶]
  F --> G[依次处理每个桶]
  G --> H[桶内插入排序]
  H --> I[合并桶中元素写回原数组; 触发 onSet]
  I --> J[完成]
```
...
```mermaid
flowchart TD
  A[CountingSort.sort] --> B[扫描数组找最大最小值]
  B --> C[计算范围 = max-min+1]
  C --> D["创建计数数组和输出数组"]
  D --> E[统计每个值的出现频率]
  E --> F[计算前缀和]
